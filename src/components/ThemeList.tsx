...
const ThemeList=()=>{
 ...
 const themes={
  ...
  const allQuestions = Object.values(themes).flatMap(theme => theme.questions);

  const displayedQuestions = activeTheme ? themes[activeTheme].questions : allQuestions;
    ...
    <section className="py-20 animate-fade-in" id="team">
      ...
      <div className="mb-8 flex flex-wrap justify-center gap-4">
        {Object.keys(themes).map((category) => (
          <Button
            key={category}
            onClick={() =>{
              console.log(`Setting active theme to: ${category}`);
      ...
      <div className="overflow-x-auto mx-10">
        <div className="flex justify-center w-[100%]">
          <table className="w-[80%] bg-[#e7fcf1] rounded-lg shadow-lg text-black">
            <thead>
            <tr>
          ...
          <td className="py-2 px-4 border-[rgb(20,184,166)] border-b border-r">
           {question.title}</td>
          <td className="py-2 px-4 border-[rgb(20,184,166)] border-b border-r">{question.PS}</td>
          <td className="py-2 px-4 border-[rgb(20,184,166)] border-b">{Object.keys(themes).find(theme => themes[theme].questions.includes(question))}</td>
                        </tr>
            ))}
          </tbody>
          </table>
